...
void main()
{
    ...
    highp vec3 quad_vertices_world_space[4] = vec3[4](rect_light_vetices[0].xyz, rect_light_vetices[1].xyz, rect_light_vetices[2].xyz, rect_light_vetices[3].xyz);
    highp vec3 area_lighting_radiance = vec3(intensity, intensity, intensity);
    ...
    highp float area_lighting_attenuation = brx_ltc_attenuation(false, quad_culling_range, P, quad_vertices_world_space);
    [[dont_flatten]]
    if (area_lighting_attenuation > INTERNAL_ATTENUATION_THRESHOLD)
    {
        ...
        brx_float3 radiance = brx_ltc_radiance(false, area_lighting_attenuation, diffuse_albedo, specular_albedo, roughness, N, V, area_lighting_radiance, P, quad_vertices_world_space);
