...
static constexpr uint32_t const INTERNAL_BRX_LTC_OPTIMIZATION_MONTE_CARLO_SAMPLE_COUNT = 16384U;
...

                {
                    ...
                    float monochromatic_brdf_cosine_value;
                    {
                        ...
                        float square_NdotH;
                        {
                            ...
                            float NdotH = std::min(std::max(0.0F, (NdotL + NdotV) * invLenH), 1.0F);
                            square_NdotH = NdotH * NdotH;
#else

                            // [BrdfGGX::eval](https://github.com/selfshadow/ltc_code/blob/master/fit/brdf_ggx.h#L34)
                            // The precision is higher when calculated based on "z*z/(x*x + y*y + z*z)", likely because "DirectX::XMVector3Normalize" and "std::sqrt" are omitted
                            double not_normalized_omega_h_x = static_cast<double>(omega_o.x) + static_cast<double>(omega_i.x);
                            double not_normalized_omega_h_y = static_cast<double>(omega_o.y) + static_cast<double>(omega_i.y);
                            double not_normalized_omega_h_z = static_cast<double>(omega_o.z) + static_cast<double>(omega_i.z);

                            double square_not_normalized_omega_h_z = not_normalized_omega_h_z * not_normalized_omega_h_z;

                            double square_length_not_normalized_omega_h = not_normalized_omega_h_x * not_normalized_omega_h_x + not_normalized_omega_h_y * not_normalized_omega_h_y + square_not_normalized_omega_h_z;

                            if (not_normalized_omega_h_z > 0.0)
                            {
                                square_NdotH = square_not_normalized_omega_h_z / square_length_not_normalized_omega_h;
                            }
                            else
                            {
                                square_NdotH = 0.0F;
                        ...
                        brdf_pdf = brx_trowbridge_reitz_pdf_omega_i_square_version(alpha, NdotV, square_NdotH);

                        monochromatic_brdf_cosine_value = brx_trowbridge_reitz_brdf_without_fresnel_square_version(alpha, square_NdotH, NdotV, NdotL) * NdotL;
                    }
...
static inline void internal_brx_linearly_transformed_cosine_optimization_compute_matrix(uint32_t const lut_width_index, uint32_t const lut_height_index, uint32_t const lut_width, uint32_t const lut_height, float &inout_M2_m11, float &inout_M2_m22, float &inout_M2_m13, DirectX::XMFLOAT4 &out_packed_M)
{
    ...
    // TODO: 16000 may be too high
    bool converged = brx_fminsearch_3d(
        16000U * 3U,
        1E-4F,
        1E-4F,
        [](DirectX::XMFLOAT3 const &x, void *wrapped_user_data) -> float
        {
            ...
            // [FitLTC::update](https://github.com/selfshadow/ltc_code/blob/master/fit/fitLTC.cpp#L131)
            // ReLU -> Sigmoid
    ...
    if (!converged)
    {
        std::printf("w %d h %d not converged\n", static_cast<int>(lut_width_index), static_cast<int>(lut_height_index));
    }
