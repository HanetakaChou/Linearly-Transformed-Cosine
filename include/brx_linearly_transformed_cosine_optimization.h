...
static constexpr float const INTERNAL_BRX_LTC_OPTIMIZATION_LENGTH_MINIMUM = 1E-5F;
...

                {
                    ...
                    float monochromatic_brdf_cosine_value;
                    {
                        ...
                        float NdotH;
                        {
                            ...
                            float invLenH = 1.0F / std::sqrt(std::max(INTERNAL_BRX_LTC_OPTIMIZATION_LENGTH_MINIMUM, 2.0F + 2.0F * VdotL));
...
static inline void internal_brx_linearly_transformed_cosine_optimization_compute_matrix(uint32_t const lut_width_index, uint32_t const lut_height_index, uint32_t const lut_width, uint32_t const lut_height, float &inout_M2_m11, float &inout_M2_m22, float &inout_M2_m13, DirectX::XMFLOAT4 &out_packed_M)
{
    ...
    float texcoord_u = (static_cast<float>(lut_width_index) + 0.5F) / static_cast<float>(lut_width);

    assert(lut_height_index < lut_height);
    float texcoord_v = (static_cast<float>(lut_height_index) + 0.5F) / static_cast<float>(lut_height);

    // u = sqrt(1 - cos_theta)
    // cos_theta = 1 - u * u
    DirectX::XMFLOAT3 omega_o;
    {
        assert((1.0F - texcoord_u * texcoord_u) >= BRX_TROWBRIDGE_REITZ_NDOTV_MINIMUM);
        float const cos_theta_o = std::max(BRX_TROWBRIDGE_REITZ_NDOTV_MINIMUM, 1.0F - texcoord_u * texcoord_u);
        omega_o = DirectX::XMFLOAT3(std::sqrt(std::max(0.0F, 1.0F - cos_theta_o * cos_theta_o)), 0.0F, cos_theta_o);
    }

    // v = ln (alpha) / ln(alpha_min)
    // alpha = exp(v * ln(alpha_min))
    float const alpha = std::exp(texcoord_v * std::log(BRX_TROWBRIDGE_REITZ_ALPHA_MINIMUM));
    ...
    DirectX::XMFLOAT3 T2;
    {
        ...
        DirectX::XMVECTOR simd_T1 = DirectX::XMVector3Normalize(DirectX::XMVectorSubtract(simd_omega_o, DirectX::XMVectorScale(simd_median_vector, DirectX::XMVectorGetX(DirectX::XMVector3Dot(simd_omega_o, simd_median_vector)))));
        DirectX::XMStoreFloat3(&T1, simd_T1);

        DirectX::XMStoreFloat3(&T2, DirectX::XMVector3Normalize(DirectX::XMVector3Cross(simd_median_vector, simd_T1)));
    }
